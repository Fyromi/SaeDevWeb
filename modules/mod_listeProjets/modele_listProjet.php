...
class ModeleListProjet extends Connexion{
    ...
    public function getList(){
        $request = connexion::$bdd->prepare('SELECT DISTINCT projet.idProjet, projet.titre, projet.description, projet.anneeUniversitaire, projet.semestre
                                                  FROM utilisateur
        ...
        $request->bindParam(':utilisateur', $utilisateur['idUtilisateur']);
        $request->execute();
    ...
    public function getProjet($id){
        $request = connexion::$bdd->prepare('SELECT DISTINCT *
                                                  FROM projet
                                                  WHERE idProjet = :id');
    ...
    public function getIDUtilisateur(){
        $request = connexion::$bdd->prepare('SELECT idUtilisateur
                                                FROM utilisateur
                                                WHERE utilisateur.login = :logi'
                                                );
        $request->bindParam(':logi', $_SESSION['login']);
        $request->execute();
    ...
    public function getProfProjet($idProjet){
        $request = connexion::$bdd->prepare("SELECT *
                                                FROM utilisateur
